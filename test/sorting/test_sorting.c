...
#define ENTRY(L, N) hvm_gen_set_debug_entry(gen->block, L, N);
...
void define_insertion_sort(hvm_gen *gen) {
  ...
  byte len     = hvm_vm_reg_gen(4);
  byte r4      = hvm_vm_reg_gen(5);
  byte r5      = hvm_vm_reg_gen(6);
  byte arr     = hvm_vm_reg_gen(7);
  byte scratch = hvm_vm_reg_gen(8);

  byte a_jminus1 = hvm_vm_reg_gen(11);
  byte a_j       = hvm_vm_reg_gen(12);
...
int main(int argc, char **argv) {
  ...
  byte results_array = hvm_vm_reg_gen(105);

  hvm_gen_arraynew(gen->block, timings_array, hvm_vm_reg_null());
  hvm_gen_arraynew(gen->block, results_array, hvm_vm_reg_null());
  ...
  hvm_gen_litinteger(gen->block, idx, 0);
  hvm_gen_litinteger(gen->block, lim, runs);
  ...
  hvm_gen_if_label(gen->block, cond, "loop_end");
    ...
    // $ret = #insertion_sort()
    hvm_gen_callsymbolic(gen->block, "insertion_sort", ret);
    // arraypush $timings_array (time_as_int())
    hvm_gen_set_symbol(gen->block, sym, "time_as_int");
    hvm_gen_invokeprimitive(gen->block, sym, timing);
    hvm_gen_arraypush(gen->block, timings_array, timing);
    // arraypush $results_array $ret
    hvm_gen_arraypush(gen->block, results_array, ret);
    // $idx = $idx + 1
    hvm_gen_litinteger(gen->block, i, 1);
  ...
  // hvm_obj_ref *arrref = hvm_vm_register_read(vm, local_array);
  ...
  */
  ...
  /*
  for(unsigned int i = 0; i < arr->array->len; i += 1) {
    hvm_obj_ref *resultref = g_array_index(arr->array, hvm_obj_ref*, i);
    hvm_obj_array *result  = resultref->data.v;

    printf("result[%d] =\n", i);
    for(unsigned int x = 0; x < result->array->len; x += 1) {
      hvm_obj_ref *intref = g_array_index(result->array, hvm_obj_ref*, x);
      int64_t _i          = intref->data.i64;
      printf("  [%4d] = %lld\n", x, _i);
    }
  }
  */
  ...
  /*
  printf("local_array[%d] =\n", arr->array->len);
  for(unsigned int x = 0; x < arr->array->len; x += 1) {
    hvm_obj_ref *intref = g_array_index(arr->array, hvm_obj_ref*, x);
    int64_t _i          = intref->data.i64;
    printf("  [%4d] = %lld\n", x, _i);
  }
  */
